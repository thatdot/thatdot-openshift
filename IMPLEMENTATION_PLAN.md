...
- `manifests/cassandra/`:
  ...
  - `serviceaccount.yaml` — dedicated `cassandra-sa` ServiceAccount + RoleBinding granting it the `anyuid` SCC. Required because cass-operator hardcodes pod `securityContext` to UID/GID 999, which `restricted-v2` (default SCC) rejects.
  - `cassandradatacenter.yaml` — `CassandraDatacenter` CR (`apiVersion: cassandra.datastax.com/v1beta1`) named `dc1`, cluster name `quine`. Single node (`size: 1`), `spec.serviceAccount: cassandra-sa` so pods get the SCC binding above, 512MB heap via `spec.config.jvm-server-options.{initial,max}_heap_size`, 2Gi PVC via `spec.storageConfig.cassandraDataVolumeClaimSpec` against `crc-csi-hostpath-provisioner` (the CRC default StorageClass).
...
- **SCC violation on Cassandra pods is guaranteed, not optional.** cass-operator (v1.23.x) sets `pod.securityContext` to UID/GID/fsGroup 999 — the standard `cassandra` user — and `restricted-v2` (the default OpenShift SCC) requires a random UID in `1000680000–1000689999` and rejects fixed values. Without the dedicated `cassandra-sa` ServiceAccount granted `anyuid`, the StatefulSet will spam `FailedCreate` events: `pods "..." is forbidden: unable to validate against any security context constraint`. The `serviceaccount.yaml` manifest is the declarative fix and is part of step 3's git-managed state. (The pod still has `runAsNonRoot: true`, so even with anyuid, root is not actually permitted.)
